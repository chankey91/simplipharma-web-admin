...
async function collectPendingMedicineIds(
  ...
  for (const doc of snap.docs) {
    ...
    if (!Array.isArray(medicines)) continue;
    for (const m of medicines) {
      const id = String(m?.medicineId || '').trim();
      if (id) ids.add(id);
    }
  }
  return ids;
...
async function searchMedicineCandidates(
  ...
  Array<{
    ...
    productId?: string;
    score: number;
    reason: 'pending_order' | 'inventory';
  }>
> {
  ...
  const hits = (res.hits || []).map((h) => (h.document || {}) as Record<string, unknown>);
  const out: Array<{
    medicineId: string;
    medicineName: string;
    productId?: string;
  ...
  for (const doc of hits) {
    ...
    const productId =
      doc.productId != null && String(doc.productId).trim()
  ...
  out.sort((a, b) => {
    if (a.reason === b.reason) return b.score - a.score;
    if (a.reason === 'pending_order') return -1;
    if (b.reason === 'pending_order') return 1;
...
async function matchVendor(
  ...
  if (gstin) {
    ...
    if (snap.size === 1) {
      const d = snap.docs[0];
      return { vendorId: d.id, vendorName: String(d.data()?.vendorName || '') };
    }
  }
...
export const processPurchaseInvoiceDraft = ff
  ...
  .https.onCall(async (data, context) => {
    ...
    if (draft.createdBy && draft.createdBy !== context.auth.uid) {
      ...
      if (isPurchaseOfficerRole(role) && draft.createdBy !== context.auth.uid) {
        throw new functions.https.HttpsError('permission-denied', 'Not your draft');
      }
    }

    const storagePath = String(draft.sourceFile?.storagePath || '').trim();
    const contentType = String(draft.sourceFile?.contentType || 'application/octet-stream');
    if (!storagePath) {
    ...
    try {
      ...
      const vendor = await matchVendor(
        db,
        extracted.vendorHint?.gstin,
        extracted.vendorHint?.name
      );
