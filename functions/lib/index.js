 ...
 */
exports.sendVendorPasswordEmailHttp = functionRegion_1.ff.https.onRequest(async (req, res) => {
 ...
 */
exports.sendVendorPasswordEmail = functionRegion_1.ff.https.onCall(async (data, context) => {
    var _a, _b, _c;
    try {
...
exports.createStoreUser = functionRegion_1.ff.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (role === 'salesOfficer' && (storeData === null || storeData === void 0 ? void 0 : storeData.alsoRetailer) === true) {
            cleanData.alsoRetailer = true;
            if (!cleanData.salesOfficerId)
                cleanData.salesOfficerId = userRecord.uid;
            if (!String(cleanData.storeCode || '').trim()) {
                try {
                    cleanData.storeCode = await (0, storeCode_1.generateNextStoreCode)();
                }
                catch (codeErr) {
 ...
 */
exports.updateRetailerEmail = functionRegion_1.ff.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    try {
 ...
 */
exports.approveRetailerRequest = functionRegion_1.ff.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    try {
 ...
 */
exports.rejectRetailerRequest = functionRegion_1.ff.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
 ...
 */
exports.onRetailerRegistrationRequestCreated = functionRegion_1.ff.firestore
 ...
 */
exports.sendPanelPasswordResetEmail = functionRegion_1.ff.https.onCall(async (data, context) => {
    var _a;
 ...
 * Requires SMTP (same as other transactional emails).
 */
exports.sendPurchaseOfficerPasswordResetEmail = functionRegion_1.ff.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    ...
    if (!(0, panelAuth_1.isPurchaseOfficerRole)(role)) {
        throw new functions.https.HttpsError('failed-precondition', 'This email is not a Purchase Officer account');
    }
    const userDoc = await admin.firestore().collection('users').doc(userRecord.uid).get();
    if (!userDoc.exists || ((_a = userDoc.data()) === null || _a === void 0 ? void 0 : _a.isActive) === false) {
    ...
    let resetLink;
    try {
        resetLink = await generatePasswordResetLinkWithFallback(email, getPurchaseOfficerPasswordResetContinueUrl(), 'sendPurchaseOfficerPasswordResetEmail');
    }
    catch (err) {
        console.error('sendPurchaseOfficerPasswordResetEmail: generatePasswordResetLink failed:', err === null || err === void 0 ? void 0 : err.message);
        throw new functions.https.HttpsError('internal', 'Could not generate password reset link');
    }
    const mail = await sendSmtpMail({
        to: email,
 ...
 * Requires SMTP (same as other transactional emails).
 */
exports.sendRetailerPasswordResetEmail = functionRegion_1.ff.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
...
exports.onSupportTicketCreated = functionRegion_1.ff.firestore
...
exports.onSupportThreadAdminMessageCreated = functionRegion_1.ff.firestore
...
var purchaseInvoiceIngest_1 = require("./purchaseInvoiceIngest");
Object.defineProperty(exports, "processPurchaseInvoiceDraft", { enumerable: true, get: function () { return purchaseInvoiceIngest_1.processPurchaseInvoiceDraft; } });
...
Object.defineProperty(exports, "adminSyncMedicineSynonymsTypesense", { enumerable: true, get: function () { return typesenseMedicines_1.adminSyncMedicineSynonymsTypesense; } });
...
var purchaseListJob_1 = require("./purchaseListJob");
Object.defineProperty(exports, "scheduledPurchaseListNoon", { enumerable: true, get: function () { return purchaseListJob_1.scheduledPurchaseListNoon; } });
Object.defineProperty(exports, "scheduledPurchaseListAfternoon", { enumerable: true, get: function () { return purchaseListJob_1.scheduledPurchaseListAfternoon; } });
Object.defineProperty(exports, "publishPurchaseListNet", { enumerable: true, get: function () { return purchaseListJob_1.publishPurchaseListNet; } });
Object.defineProperty(exports, "syncPurchaseManufacturerToOrders", { enumerable: true, get: function () { return purchaseListJob_1.syncPurchaseManufacturerToOrders; } });
Object.defineProperty(exports, "onPurchaseListManufacturerSubmit", { enumerable: true, get: function () { return purchaseListJob_1.onPurchaseListManufacturerSubmit; } });
