  ...
  const handleRunNow = async () => {
    ...
    try {
      ...
      if (result.itemCount === 0) {
        ...
        await alert(
          `Published ${result.itemCount} medicines (net remaining).` +
            (eliminated || reduced
              ? ` Eliminated ${eliminated} covered; reduced ${reduced} partially covered.`
              : '') +
            ' Prior open lists superseded.',
          { severity: 'success' }
        );
      }
    } catch (err: unknown) {
    ...
    <Box>
      ...
      <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={2}>
        <Typography variant="body2" color="text.secondary" sx={{ maxWidth: 720 }}>
          Auto-published daily at 12:00 and 15:00 IST from today&apos;s pending orders.
          Each run uses net remaining need (already-found quantities are excluded).
        </Typography>
        <Button
          variant="contained"
          size="small"
